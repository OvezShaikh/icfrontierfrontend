...
export default function PublicBlogPage() {
  ...
  const fetchPosts = useCallback(async () => {
    try {
      ...
      const res = await axios.get(`${config.API_URL}/api/posts/public`); 
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-blue-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 py-12 lg:py-24">
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        ...
        {loading ? (
          ...
          >
            <div className="w-20 h-20 border-4 border-blue-200/50 border-t-blue-600 rounded-full animate-spin mb-8"></div>
            <p className="text-2xl text-gray-600 dark:text-gray-400">Loading posts...</p>
          </motion.div>
          ...
          >
            ...
            <p className="text-xl text-gray-600 dark:text-gray-400 max-w-lg mx-auto">
              {searchTerm ? "Try different search terms." : "No blog posts available yet."}
            </p>
          </motion.div>
        ) : (
          <motion.div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-8 lg:gap-10">
            {filteredPosts.map((post, index) => (
              <motion.article
                ...
                whileHover={{ y: -12 }}
                className="group relative overflow-hidden rounded-3xl bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-gray-200/50 shadow-2xl hover:shadow-3xl hover:bg-white transition-all duration-500 cursor-pointer"
              >
                {post.image && (
                ...
                <div className="p-8 lg:p-10">
                  ...
                  <h3 className="text-2xl lg:text-3xl font-bold mb-4 text-gray-900 dark:text-white line-clamp-2">
                    {post.title}
                  </h3>
